...
**Plotsar is a plotting package designed exclusively for [Prometheus](https://github.com/XGI-MSU/prometheus) PTA analysis outputs.** It is not a general-purpose plotting library — it is built around Prometheus's `PTAModel` object and the MCMC sample dictionaries that Prometheus produces. Plotsar reads your model directly to figure out pulsar names, sky locations, parameter shapes, and constituent sub-models (`psr_model`, `gwb_model`, `det_model`), so you spend your time looking at results rather than wrangling labels and array indices.

> **Requirement:** Plotsar expects a Prometheus `PTAModel` instance and a sample dictionary as returned by NumPyro / the Prometheus utility functions. It will not work meaningfully without them.
...
```bash
pip install git+https://github.com/aiden-g12321/plotsar.git
```

To install an editable local copy (e.g. while developing alongside Prometheus):

```bash
git clone https://github.com/aiden-g12321/plotsar.git
cd plotsar
pip install -e .
```
...
Because Plotsar has access to the `PTAModel`, you never need to type pulsar names or remember parameter orderings. Use the model-level helpers to select parameters by which sub-model they belong to:
...
Supply a `truths` dict at construction time using the same key-value format as the samples dictionary. Plotsar converts it automatically to the flat catalog format and uses it as the default for all subsequent plots:
...
plotter = Plotsar(model, samples, truths=truths)
...
Plotsar infers parameter names from array shape (8-param → standard CW names, 2-param → power-law names). Override with `param_names`:

```python
plotter = Plotsar(model, samples, param_names={
